...
//------------------------------------
{
    ...
    int id = 0;
    float temp0 = float(mod(float(iz), tab_size));
    id = int(mod(float(ix + g_perm[int(mod(float(iy+g_perm[int(temp0)]), tab_size))]), tab_size));
	return g_perm[id];
}
...
//------------------------
{
	...
	if(g_has_tendon == 1)
	{
		...
		bump_value+=0.1*(1.0-horizontal)*
			(1.0-(pow(abs(sin(5.0*LINE_FREQ*(theta+perturbation))),14.0)+
			0.6*pow(abs(sin(7.0*LINE_FREQ*(theta+perturbation))),10.0))+
			0.4*pow(abs(sin(9.0*LINE_FREQ*(theta+perturbation))),8.0));
	}
...
//-------------
{
	...
	gl_FragColor = vec4(deltaNormal*0.5+0.5, 1.0);
}
